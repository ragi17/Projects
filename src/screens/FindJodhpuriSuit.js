...
export default class FindJodhpuriSuit extends Component {
  constructor(props) 
  {
      super(props);
      this.state = 
  ...
  componentDidMount() {
    ...
    firestore().collection('Customer').doc(id.uid).get().then((docRef) => {
      // let name=docRef.data().Name;
        ...
        this.setState({Collar: docRef.data().Collar}),
        ...
        this.setState({SuitLength: docRef.data().SuitLength})
      });
    console.log(id.uid)
  ...
  nilesh() {
    firestore().collection('Tailor').doc("hZaT74xIKPOdDmrcAbyyV74HUkk1").update({
      ...
      SuitLength:this.state.SuitLength
    });
