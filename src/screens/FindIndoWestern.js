...
export default class FindIndoWestern extends Component {
  constructor(props) 
  {
      super(props);
      this.state = 
  ...
  componentDidMount() {
    ...
    firestore().collection('Customer').doc(id.uid).get().then((docRef) => {
      // let name=docRef.data().Name;
        ...
        this.setState({OBust: docRef.data().OBust}),
        this.setState({UBust: docRef.data().UBust}),
        this.setState({Waist: docRef.data().Waist}),
        this.setState({Arm: docRef.data().ArmLength}),
