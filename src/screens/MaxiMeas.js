...
export default class MaxiMeas extends Component {
  constructor(props) 
  {
      super(props);
      this.state = 
  ...
  measurement() {
    if(this.state.neck && this.state.bust && this.state.obust && this.state.ubust && this.state.waist && this.state.arm && this.state.armhole && this.state.aroundbicep && this.state.forearm && this.state.wrist && this.state.shoulder === "" ) {
  ...
  { 
    return(  
      ...
      <TextInput keyboardType='numeric' placeholder='Fore Arm' style={measstyles.shoulder} onChangeText={(Text) => this.setState({forearm: Text})} value={this.state.froearm}/>
