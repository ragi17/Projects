...
export default class FindShervani extends Component {
  constructor(props) 
  {
      super(props);
      this.state = 
  ...
  componentDidMount() {
    ...
    firestore().collection('Customer').doc(id.uid).get().then((docRef) => {
      // let name=docRef.data().Name;
        ...
        this.setState({ArmLength: docRef.data().ArmLength}),
        this.setState({Hips: docRef.data().Hips}),
        this.setState({ShervaniLength: docRef.data().ShervaniLength}),
  ...
  nilesh() {
    firestore().collection('Tailor').doc("hZaT74xIKPOdDmrcAbyyV74HUkk1").update({
      ...
      ShervaniLength:this.state.ShervaniLength,
