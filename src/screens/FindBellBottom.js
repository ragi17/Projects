...
export default class FindBellBottom extends Component {
  constructor(props) 
  {
      super(props);
      this.state = 
  ...
  componentDidMount() {
    ...
    firestore().collection('Customer').doc(id.uid).get().then((docRef) => {
      // let name=docRef.data().Name;
        ...
        this.setState({BellBottomLength: docRef.data().BBLength}),
  ...
  nilesh() {
    firestore().collection('Tailor').doc("hZaT74xIKPOdDmrcAbyyV74HUkk1").update({
      ...
      category: 'Bell Bottom',
      Inseam: this.state.Inseam,
      BellBottomLength: this.state.BellBottomLength,
